...
function Module:CaptureEvent(EncodedPayload)
	if not self.InitThread then return self:_Relay("CaptureEvent", EncodedPayload) end
...
function Module:CaptureEnvelope(Payload)
	if not self.InitThread then return self:_Relay("CaptureEnvelope", Payload) end
...
function Module:Init(Options, SENTRY_PROTOCOL_VERSION, SENTRY_CLIENT)
	assert(
		not script:FindFirstChildWhichIsA("BindableFunction"),
	...
	do --/ Process Options
		local Scheme, PublicKey, Authority, ProjectId =
			string.match(Options.DSN or "", "^([^:]+)://([^:]+)@([^/]+)/(.+)$")
